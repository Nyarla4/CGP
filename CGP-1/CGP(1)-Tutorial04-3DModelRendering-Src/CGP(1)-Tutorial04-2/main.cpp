...
int main()
{
	bool result;
	char filename[256];
	int vertexCount, textureCount, normalCount, faceCount;//버텍스, 텍스쳐, 노말, 페이스 개수를 읽어들인다
	char garbage;
	...
	result = ReadFileCounts(filename, vertexCount, textureCount, normalCount, faceCount);//일단 개수를 카운트(나중에 다시 불러서 정보를 가져온다)
	if(!result)
	{
		return -1;
	}
	...
	result = LoadDataStructures(filename, vertexCount, textureCount, normalCount, faceCount);//실제 정보를 가져오는 부분
	if(!result)
	{
		return -1;
	}
