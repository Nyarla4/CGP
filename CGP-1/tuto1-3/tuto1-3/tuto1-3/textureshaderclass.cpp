////////////////////////////////////////////////////////////////////////////////
// Filename: textureshaderclass.cpp//그래픽 클래스 하위, ps,vs파일 상위
////////////////////////////////////////////////////////////////////////////////
...
/*렌더ㅓ*/						D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture)
{
	bool result;
...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	polygonLayout[1].SemanticName = "TEXCOORD";//컬러->텍스쳐코드
	polygonLayout[1].SemanticIndex = 0;
	polygonLayout[1].Format = DXGI_FORMAT_R32G32_FLOAT;//4바이트짜리
	...
	// settings for the sampler state description are defaults.//이하가 새로 추가된 
	samplerDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR;//trilinear filter의 경우이다.  MIP_POINT:Point필터, LINEAR_MIP_POINT:Bilinear필터, MIP_LINER:Trilinear필터(현재 설정)
    samplerDesc.AddressU = D3D11_TEXTURE_ADDRESS_WRAP;
    samplerDesc.AddressV = D3D11_TEXTURE_ADDRESS_WRAP;
    samplerDesc.AddressW = D3D11_TEXTURE_ADDRESS_WRAP;
    samplerDesc.MipLODBias = 0.0f;
    samplerDesc.MaxAnisotropy = 1;//필터가 anisotrophy의 경우 설정해야한다. 2,4,8,16 중에서.
											 ...
											 D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture)
{
	...
	result = deviceContext->Map(m_matrixBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);//넘겨준다! 매핑? 아마 그거?
	if(FAILED(result))
	{
		return false;
	}
    ...
    deviceContext->Unmap(m_matrixBuffer, 0);//넘겨줬으니까 언맵!!
	//☆★ 셰이더 내의 데이터는 16비트로 끊어서 써야한다! 한계가 있다! 16이 안되면 뒤에 좀 더 붙여야해
	// Set the position of the constant buffer in the vertex shader.
	bufferNumber = 0;

	// Now set the constant buffer in the vertex shader with the updated values.
    deviceContext->VSSetConstantBuffers(bufferNumber, 1, &m_matrixBuffer);//매트릭스 버퍼 넘기기

	// Set shader texture resource in the pixel shader.
	deviceContext->PSSetShaderResources(0, 1, &texture);//텍스쳐 정보 넘기기
...
void TextureShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
	...
	deviceContext->PSSetSamplers(0, 1, &m_sampleState);//샘플이다. 앞은 그냥 0,1하는게 속편하다
