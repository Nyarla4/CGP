...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	result = m_Model->Initialize(m_D3D->GetDevice(), (CHAR*)"./data/cube.txt", (WCHAR*)L"./data/seafloor.dds");
...
bool GraphicsClass::Render(float rotation)
{
	...
	D3DXMatrixRotationY(&worldMatrix, rotation);//회전시키는 부분, 월드매트릭스에 회전값을 얼마나 줄것인가

	// Put the model vertex and index buffers on the graphics pipeline to prepare them for drawing.
	m_Model->Render(m_D3D->GetDeviceContext());//모델 렌더는 나중
	...
	result = m_TextureShader->Render(m_D3D->GetDeviceContext(), m_Model->GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix, 
									 m_Model->GetTexture());//더 나중에 하는 게 텍스쳐 렌더
	if(!result)
	{
		return false;
	}
