...
bool ParticleSystemClass::Initialize(ID3D11Device* device, const WCHAR* textureFilename,int index)
{
	...
	// 파티클 시스템을 초기화합니다.
	if (!InitializeParticleSystem(this->index))
	{
		return false;
	}
...
void ParticleSystemClass::EmitParticles(float frameTime)
{
	...
	if ((emitParticle == true) && (m_currentParticleCount < (m_maxParticles - 1)))
	{
		...
		{
		case 1:
			red = 1.0f;	green = 1.0f;	blue = 0.75f;
			break;
		case 2:
			red = 0.50f;	green = 0.75f;	blue = 0.75f;
			break;
		default:
			red = 1.0f;	green = 1.0f;	blue = 0.75f;
			break;
...
{
	// 각 프레임은 위치, 속도 및 프레임 시간을 사용하여 모든 파티클을 업데이트합니다.
...
void ParticleSystemClass::KillParticles()
{
	...
	for (int i = 0; i < m_maxParticles; i++)
	{
		if ((m_particleList[i].active == true) && (m_particleList[i].positionY > height))
		{
			m_particleList[i].active = false;
			m_currentParticleCount--;
