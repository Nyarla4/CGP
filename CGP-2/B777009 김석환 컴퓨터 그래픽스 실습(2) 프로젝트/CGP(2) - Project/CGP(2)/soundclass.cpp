...
bool SoundClass::Initialize(HWND hwnd, char* bgmfile, char* SEfile, char* SEfile2)//, char* Wfile, char* Lfile)
{
	bool result;
	...
	result = LoadWaveFile(SEfile2, &m_secondaryBuffer2_5);
	if (!result)
	{
		return false;
	}
...
void SoundClass::Shutdown()
{
	...
	ShutdownWaveFile(&m_secondaryBuffer2_5);
...
bool SoundClass::PlaySEFile2()
{
	HRESULT result;


	// Set position at the beginning of the sound buffer.
	result = m_secondaryBuffer2_5->SetCurrentPosition(0);
	if (FAILED(result))
	{
		return false;
	}
	...
	// Set volume of the buffer to 100%.
	result = m_secondaryBuffer2_5->SetVolume(DSBVOLUME_MAX);
	if (FAILED(result))
	{
		return false;
	}
	...
	// Play the contents of the secondary sound buffer.
	result = m_secondaryBuffer2_5->Play(0, 0, 0);
	if (FAILED(result))
	{
		return false;
	}
